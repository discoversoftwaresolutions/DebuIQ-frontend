...
# AND that you have empty **init**.py files in:

# - DebuIQ-frontend/frontend/**init**.py

# - DebuIQ-frontend/.screens/**init**.py

autonomous\_tab\_imported = False
show\_autonomous\_tab\_import\_error = None # Variable to store potential import error message
show\_autonomous\_workflow\_tab = None
try:
\# Using relative import based on your structure: go up one level (..) then into .screens
\# Ensure **init**.py files exist in both 'frontend' and '.screens' relative to DebuIQ-frontend/
from ..screens.AutonomousWorkflowTab import show\_autonomous\_workflow\_tab
autonomous\_tab\_imported = True
except ImportError as e:
\# Do NOT use st.error here, just set the flag and store the error message
autonomous\_tab\_imported = False
show\_autonomous\_tab\_import\_error = str(e) # Store the error message
show\_autonomous\_workflow\_tab = None # Ensure it's None

# --- Streamlit Page Configuration ---

# set\_page\_config() MUST be the very first Streamlit command after imports

st.set\_page\_config(page\_title="DebugIQ Dashboard", layout="wide")

st.title("🧠 DebugIQ Autonomous Debugging Dashboard")

# --- Display import error *after* set\_page\_config ---

# This message will only show if the import failed earlier

if not autonomous\_tab\_imported:
st.error(f"Could not import the Autonomous Workflow Orchestration tab: {show\_autonomous\_tab\_import\_error}. "
f"Make sure AutonomousWorkflowTab.py is at the correct path (DebuIQ-frontend/.screens/) "
f"and **init**.py files are in the 'frontend' and '.screens' directories.")

# Corrected the syntax error in os.getenv by adding quotes around the default URL

BACKEND\_URL = os.getenv("BACKEND\_URL", "[https://debugiq-backend.onrender.com](https://debugiq-backend.onrender.com)")

@st.cache\_data
def fetch\_config(backend\_url): # Pass backend\_url to cache function dependencies
"""Fetches backend configuration, error handling is done after the call."""
try:
r = requests.get(f"{backend\_url}/api/config")
r.raise\_for\_status() # Raise an HTTPError for bad responses (4xx or 5xx)
return r.json()
...
# --- Fetch and Display Config (Moved after set\_page\_config) ---

config = fetch\_config(BACKEND\_URL) # Call the function

if config:
st.sidebar.info("Backend Config Loaded:")
st.sidebar.json(config) # Display full config for debugging
\# st.sidebar.info(f"🔧 Voice Provider: {config.get('voice\_provider', 'N/A')}") # Specific info if needed
...
# Use fetched URLs, falling back to defaults constructed with BACKEND\_URL

ANALYZE\_URL = config.get("analyze\_url", f"{BACKEND\_URL}/debugiq/analyze") if config else f"{BACKEND\_URL}/debugiq/analyze"
QA\_URL = config.get("qa\_url", f"{BACKEND\_URL}/qa/") if config else f"{BACKEND\_URL}/qa/"
TRANSCRIBE\_URL = config.get("voice\_transcribe\_url", f"{BACKEND\_URL}/voice/transcribe") if config else f"{BACKEND\_URL}/voice/transcribe"
COMMAND\_URL = config.get("voice\_command\_url", f"{BACKEND\_URL}/voice/command") if config else f"{BACKEND\_URL}/voice/command"

# --- Session State Initialization ---

if 'analysis\_results' not in st.session\_state:
st.session\_state.analysis\_results = {
'trace': None,
'patch': None,
'explanation': None,
'doc\_summary': None,
'patched\_file\_name': None,
'original\_patched\_file\_content': None,
'source\_files\_content': {}
}
if 'qa\_result' not in st.session\_state:
st.session\_state.qa\_result = None

# === GitHub Repo Integration Sidebar ===

st.sidebar.markdown("### 📦 Load From GitHub Repo")
repo\_url = st.sidebar.text\_input("Public GitHub URL", placeholder="[https://github.com/user/repo](https://github.com/user/repo)", key="github\_repo\_url\_input") # Changed key
...
uploaded\_files = st.file\_uploader("📄 Upload traceback (.txt) + source files", type=\["txt", "py"], accept\_multiple\_files=True, key="manual\_file\_uploader") # Added key

if uploaded\_files:
trace\_content\_upload = None
source\_files\_content\_upload = {}
for file in uploaded\_files:
content = file.getvalue().decode("utf-8")
if file.name.endswith(".txt"):
trace\_content\_upload = content
else:
source\_files\_content\_upload\[file.name] = content # Store with original filename
...
if st.button("🧠 Run DebugIQ Analysis", key="run\_analysis\_button"): # Added key
if st.session\_state.analysis\_results.get('trace') is None and not st.session\_state.analysis\_results.get('source\_files\_content'): # Use .get()
st.warning("Please upload a traceback or source files from the sidebar or below.")
else:
with st.spinner("Analyzing with GPT-4o..."):
try:
res = requests.post(ANALYZE\_URL, json={
"trace": st.session\_state.analysis\_results.get('trace'), # Use .get()
"language": "python", # Assuming python, adjust if needed
"config": {}, # Pass relevant config if needed
"source\_files": st.session\_state.analysis\_results.get('source\_files\_content', {}) # Use .get()
})
...
if st.button("🛡️ Run QA on Patch", key="run\_qa\_button"): # Added key
if st.session\_state.analysis\_results.get('patch') is None:
st.warning("Please run analysis and generate a patch first.")
elif st.session\_state.analysis\_results.get('patched\_file\_name') is None:
st.warning("Analysis results are missing the patched file name. Rerun analysis.")
else:
with st.spinner("Running QA..."):
try:
qa\_res = requests.post(QA\_URL, json={
"trace": st.session\_state.analysis\_results.get('trace'),
"patch": st.session\_state.analysis\_results\['patch'], # Use potentially edited patch
"language": "python", # Assuming python
"source\_files": st.session\_state.analysis\_results.get('source\_files\_content', {}),
"patched\_file\_name": st.session\_state.analysis\_results\['patched\_file\_name']
})
if qa\_res.status\_code == 200:
qa\_data = qa\_res.json()
st.session\_state.qa\_result = qa\_data
st.success("✅ QA complete")
else:
st.error(f"QA failed: {qa\_res.status\_code}")
st.error(f"Response body: {qa\_res.text}")
st.session\_state.qa\_result = None # Clear previous result on failure
except requests.exceptions.RequestException as e:
st.error(f"Error communicating with backend: {e}")
st.session\_state.qa\_result = None # Clear previous result on failure
...
doc\_summary = st.session\_state.analysis\_results.get("doc\_summary", "No documentation available.")
st.markdown(doc\_summary)

with tab4:
st.subheader("📥 Issue Inbox")
\# Using a button in the main area to refresh is clearer
if st.button("🔄 Refresh Inbox", key="refresh\_inbox\_button"):
\# Clearing cache\_data *might* be too broad. Better to manage state in session\_state.
\# st.cache\_data.clear() # Consider removing if only session\_state is used for inbox\_data
st.session\_state.inbox\_data = None # Clear session state cache
...
if autonomous\_tab\_imported: # Only call if the import was successful
\# Pass the BACKEND\_URL to the tab function
show\_autonomous\_workflow\_tab(BACKEND\_URL)
...
if st.button("🔄 Refresh Status", key="refresh\_status\_button"): # Added key
st.session\_state.workflow\_status = None # Clear session state cache
\# st.cache\_data.clear() # Consider removing
...
# The webrtc\_streamer component handles its own state and UI (Start/Stop button)

# Use a key based on BACKEND\_URL so the component resets if the backend changes

ctx = webrtc\_streamer(
key=f"voice\_agent\_stream\_{BACKEND\_URL}", # Use a unique key, dependent on URL
mode=WebRtcMode.SENDONLY, # Send audio from browser to server
client\_settings=ClientSettings(
media\_stream\_constraints={"audio": True, "video": False},
\# Use a list for iceServers, even if only one
rtc\_configuration={"iceServers": \[{"urls": \["stun\:stun.l.google.com:19302"]}]}
),
\# audio\_receiver\_size parameter is deprecated
)

# Process audio frames if the connection is active and audio\_receiver is available

# Use st.session\_state to buffer frames if necessary across reruns

if "audio\_buffer" not in st.session\_state:
st.session\_state.audio\_buffer = b""
if "audio\_frame\_count" not in st.session\_state:
st.session\_state.audio\_frame\_count = 0
if "audio\_sample\_rate" not in st.session\_state:
st.session\_state.audio\_sample\_rate = 16000 # Default common sample rate
if "audio\_sample\_width" not in st.session\_state:
st.session\_state.audio\_sample\_width = 2 # Default 16-bit audio (2 bytes)
if "audio\_num\_channels" not in st.session\_state:
st.session\_state.audio\_num\_channels = 1 # Default mono

if ctx and ctx.audio\_receiver:
try:
\# Get frames with a small timeout
audio\_frames = ctx.audio\_receiver.get\_frames(timeout=0.1)
